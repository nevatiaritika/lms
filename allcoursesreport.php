...
$Line ++;
$Line ++;
$F->setCellValue ( "A" . $Line, "ACTIVE COURSES" );
$Line ++;
$Line ++;
$db = mysqli_connect ( $dburl, $dbuser, $dbpassword, $dbdatabase ) or die ( "Can't connect to database!" );
$query = 'SELECT DISTINCT cid FROM user_assign WHERE cid IN (SELECT cid FROM courses WHERE active = 1) ORDER BY STR_TO_DATE(dateassign, "%Y-%m-%d")';
...
for($coursecount = 0; $coursecount < sizeof ( $cids ); $coursecount ++) {
	$cid = $cids [$coursecount];
	...
	// echo $startdate . " " . $enddate . " " . $dept . " " . $post;
...
$Line ++;
$Line ++;
$F->setCellValue ( "A" . $Line, "INACTIVE COURSES" );
$Line ++;
$Line ++;

$query = 'SELECT DISTINCT cid FROM user_assign WHERE cid IN (SELECT cid FROM courses WHERE active = 0) ORDER BY STR_TO_DATE(dateassign, "%Y-%m-%d")';
$stmt = mysqli_prepare ( $db, $query );
mysqli_stmt_execute ( $stmt );
mysqli_stmt_store_result ( $stmt );
mysqli_stmt_bind_result ( $stmt, $CID );
